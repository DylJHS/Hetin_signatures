# This script is the third part of the cancer-specific meta model prediction pipeline.
...
# This file should be run from the command line using parallel processing specifying the index of the cancer type to tune for
# The index should be an integer from 1 - 4
...
# Set the index for the cancer type to tune for from the command line
args <- commandArgs(trailingOnly = TRUE)
index <- as.numeric(args[1]) # for the cancer index
...
# Load the full base predictions
...
for (feature in response_features) {
  ...
  if (!file.exists(hyperparameter_file)) { # If the hyperparameters file does not exist
  # Set the default hyperparameters; these can be adjusted manually
    ...
    if (!(feature %in% hyperparameters$Feature)) { # If the feature is not in the hyperparameters set for the cancer type
    # Use default hyperparameters; these can be adjusted manually
      ...
      selected_min_child <- 2
      selected_seed <- 99
    } else { # If the feature is in the hyperparameters set for the cancer type
      # Use the hyperparameters from the file; these can be adjusted manually
...
# Get the best hyperparameter values for each feature
