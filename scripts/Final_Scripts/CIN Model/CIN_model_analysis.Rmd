---
title: "General model prediction, feature importance analysis"
output: html_document
date: "2024-06-27"
note: "This file is intended to be used for the full base & meta layer predictions of the CIN model 
using the soi RNAseq data to the final learner interactions feature importance analysis"
---
...
LOAD AND DEFINE RESPONSE FEATURES
```{r}
...
CREATE THE FULL CIN FEATURE DF
```{r}
# Merge the numerical features
reg_cin <- merge(
  hrd,
  peri_cnv,
  by = "row.names"
...
# Get all response feature names
...
```{r}
# Create the folds to be used for the 5 fold cross-validation
...
for (feature in response_features) {
  ...
  # Merge the RNA data with the CIN data
  full_df <- merge(rna_set, full_cin, by = "row.names")
  
  # Create the response feature vector
  y <- ifelse(feature %in% cat_features, as.integer, as.numeric)(full_df[[feature]])
  cat("\n y: ")
  print(head(y, 15))
  
  # Remove the response feature from the dataframe
  X <- full_df %>%
...
# reform the base_oof_predictions using the act_index as the rownames
...
General genomic (Base_layer) Feature Importance Plot
```{r}
# import the feature importance df if needed or use the one from the previous step
full_feature_imp <- read.csv(
  "Full_base_feature_importance.csv",
  row.names = 1
)
...
```{r}
# Upload the base predictions if needed or use the one from the 2 steps above
...
META LAYER TRAINING & PREDICTIONS
```{r}
...
# Create the label data for the meta learners
...
for (feature in response_features) {
  ...
  # Check if parameters were found, if not skip the feature
  if (nrow(selected_parameters) == 0) {
    cat("No parameters found for feature: ", feature, "\n")
    next
  }
  ...
  for (fold_index in seq_along(folds)) {
    ...
    # Define the class weights for the categorical features
    if (feature %in% cat_features) {
      selected_ref <- as.numeric(
        selected_parameters[c(
          "Weight_loss",
  ...
  # Store the genomic feature importance df
...
write.csv(
  meta_oof_predictions,
  "Full_meta_predictions.csv"
)

# Save the feature importance df
write.csv(
  full_feature_imp_df,
  "Full_meta_feature_importance.csv"
)
```
...
# Heatmap of the top features
top_learners_matrix <- full_feature_imp_df %>%
  mutate(Gain = round(Gain, 3)) %>% 
  dplyr::select(Feature, Target, Gain) %>%
  pivot_wider(names_from = Feature, values_from = Gain) %>%
  replace(is.na(.), 0) 

top_learners_long <- top_learners_matrix %>% 
...
write.csv(
  top_learners_long,
  "Top_baselearners_complete.csv"
)
  
# Calculate the Uniform Gain Threshold (1/number of base learners)
midpoint <- round(1 / length(unique(top_learners_long$Target)),3)

heatmap_plot <- ggplot(top_learners_long, aes(x = Feature, y = Target, fill = Gain)) +
  geom_tile(colour = "black", lwd = 0.3) +
  scale_fill_gradient2(low = "white", mid = "white", high = "#D21717", midpoint = midpoint) +
...
```{r}
# Upload the meta predictions if needed 
...
# Upload the base predictions if needed
base_oof_predictions <- read.csv(
  paste0(
    input_path,
    "Base_predictions/Full_base_predictions.csv"
...
# Save the plot
ggsave(
  filename = "Plots/Model_Accuracy_line.pdf",
  plot = cat_acc_plot,
  width = 10,
  height = 5
)
...
```{r}
# Load the regression metrics if needed
...
# Save the plot
ggsave(
  filename = "Plots/Model_Rsquared_line.pdf",
  plot = reg_rsqd_plot,
  width = 10,
  height = 5
)
