---
title: "Cancer-specific model prediction, feature importance analysis"
output: html_document
date: "2024-06-27"
note: "This file is intended to be used for the base and meta layer predictions of the cancer-specific CIN models 
...
# Select the desired cancer type from the list
cancer_type <- "LUSC"
...
input_path <- "Cancer_specific_data/Model_input"

# Set the path for the outputs
output_path <- "Cancer_specific_data/Model_output"
...
for (feature in response_features[1:length(response_features)]) {
  ...
  X <- full_df %>%
    dplyr::select(-c("Row.names", all_of(all_response_features))) %>%
    as.data.frame()
...
write.csv(
  ...
  file.path(
    output_path,
    cancer_type,
    "Results/Feature_importance/Top_genes_complete.csv"
  )
)
...
genomic_heatmap <- ggplot(top_genes_complete, aes(x = Feature, y = Target, fill = Gain)) +
  ...
  labs(title = "",
       x = "Gene",
       y = "Base Learner",
       caption = paste0("Uniform Gain Threshold: ", midpoint),
...
ggplot2::ggsave(
  filename = paste0(
    ...
    "/",
    cancer_type,
    "/Plots/Feature_importance/Base_feature_importance.pdf"
  ),
...
write.csv(
  ...
  paste0(
    ...
    "/",
    cancer_type,
    "/Results/Feature_importance/Top_baselearners_complete.csv"
  )
)
...
```{r}
# Upload the meta predictions if not already loaded
...
cat_acc_plot <- ggplot(
  ...
  scale_y_continuous(
    limit = c(ifelse(min(Cat_general_metrics$Accuracy) > 0.25,
