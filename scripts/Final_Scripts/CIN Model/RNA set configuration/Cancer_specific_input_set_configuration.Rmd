...
Construct the new cancer folders
```{r}
# Path to RNA
RNA_path <- ""
...
```{r}
# Loop over the RNA csv sets in the folders
for (set in list.files(data_folder, pattern = ".csv")) {
  ...
  for (cancer in unique(rna_set$Cancer)) {
    ...
    cancer_rna_set <- rna_set %>% 
      ...
      select(-Cancer)
    # Check if the cancer RNA type subset is empty
    if (nrow(cancer_rna_set) == 0) {
      next # if so skip to the next cancer type
    }
    cat("\t Number of samples: ", nrow(cancer_rna_set), "\n")
    # else create a new folder for the cancer type
    subfolder <- paste0("Data/Cancer_specific_data/Model_input/RNA/", cancer, "/Full")
    if (!dir.exists(subfolder)) {
      dir.create(subfolder)
    }
    
    # and Save the RNA set
