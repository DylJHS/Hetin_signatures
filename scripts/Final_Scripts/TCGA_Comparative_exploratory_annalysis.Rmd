...
date: "2024-02-29"
Note: This is the Comparative Exploratory analysis using solely the TCGA data, both cancerous and non-cancerous, without the data acquisition and wrangling steps which are performed in the python scripts.
---
...
```{r}
# Calculate the mean expression levels for each sample across all genes in the SOI for both the cancerous and healthy data
...
```{r}
# Create a table that contains all th differences between the set of interest and control set, for each control set for the healthy samples
nrm_Dif_table <- data.frame(id = rownames(nrm_soi_sum))
# Create a table that contains all the SOI and Control_set global expression means for the healthy samples
nrm_Mean_table <- data.frame(id = rownames(nrm_soi_sum), soi_mean = nrm_soi_sum$soi_geom_mean)

# Create a table that contains all th differences between the set of interest and control set, for each control set for the cancerous samples
can_Dif_table <- data.frame(id = rownames(can_soi_sum))
# Create a table that contains all the SOI and Control_set global expression means for the cancerous samples
...
for (i in 1:num_datasets) {
  ...
  # Read in the healthy control set data
  ...
  # Calculate the summary statistics for the healthy control set
  nrm_ctrl_summary <- calculate_summary(ctrl_set_nrm)

  # Merge the summary statistics for the healthy control set with the summary statistics for the healthy SOI
  ...
  # Add the results to the difference and mean tables
...
print(can_Contrast_agg_count_table)

nrm_Contrast_agg_count_table <- nrm_Contrast_table %>% group_by(Significant) %>% dplyr::summarise(count = n())
print(nrm_Contrast_agg_count_table)
```

Result: The Contrast analysis shows that the expression pattern of the control gene-sets differs significantly compared to the average for  540/547 in the cancerous data and   526/547 in the healthy data, with an adjusted p-value < 0.01 using the paired Wilcoxon Signed Rank Test. 
...
Interpretation: the p-value generated by the two-sided paired permutation test demonstrates a significant difference between the means of the SOI and the Control_set for each of the control_sets. Indicating that the general expression of the genes of interest is reliably and significantly different than the general expression from the rest of the genome in both cancer and healthy tissues. 
...
Interpretation: The positive difference in average expression between the SOI and the control gene-set is maintained across all cancer types and all tissue types in the healthy data.
...
Interpretation: Indicates that the Expression pattern of positive ∆ mean Expression between the SOI and the control gene-set is present across all cancer types and tissue types for the healthy data and that there are some differences in these levels. 
...
II) healthy vs Cancerous Combined Analysis
...
Build the metrics dfs for both the tumour and the healthy data and merge
...
B)  SOI cancer-healthy expression differences Boxplots to visualise the
...
of the SOI when shifting from the healthy samples to the cancerous ones.
...
the cancerous and healthy conditions is significant with a p_value of
...
Control Sets cancer-healthy expression comparison
...
D) Perform pairwise wilcoxon signed rank test using the paired expression
   levels of the different control sets across conditions (healthy/cancerous)
...
pdf("/Users/Dyll/Documents/Education/VU_UVA/Internship/Epigenetics/Janssen_Group-UMCUtrecht/Main_Project/Plots/RNA_Plots/RNA_Plts_Part_I/Sets Volcano Plots/Full_Volcano.pdf", width = 10, height = 10)
print(Full_Volcano)
dev.off()
