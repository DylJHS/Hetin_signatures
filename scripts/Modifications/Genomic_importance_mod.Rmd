...
for (cancer in can_types) {
  ...
  feat_imp_folder <- file.path(feat_imp_folder_pre, cancer, feat_imp_post)
  ...
  complete_gene_order <- heatmap_genes_complete %>%
    group_by(Feature) %>%
    summarise(
  ...
  heatmap_genes_complete$Feature <- factor(
    heatmap_genes_complete$Feature,
    levels = complete_gene_order
  )
  ...
  rm(list = c("heatmap_genes_complete_name", "heatmap_genes_complete", "complete_genomic_heatmap_name", "complete_genomic_heatmap"))
  ...
  # Set the order for the genes
  arm_filt_gene_order <- heatmap_genes_arm_filt %>%
    group_by(Feature) %>%
    summarise(
      features_impacted = sum(Gain > 0, na.rm = TRUE),
      median_impact = median(Gain, na.rm = TRUE)
  ...
  heatmap_genes_arm_filt$Feature <- factor(
    heatmap_genes_arm_filt$Feature,
    levels = arm_filt_gene_order
  )
  ...
  # Set the order for the genes
  chrm_filt_gene_order <- heatmap_genes_chrm_filt %>%
    group_by(Feature) %>%
    summarise(
      features_impacted = sum(Gain > 0, na.rm = TRUE),
      median_impact = median(Gain, na.rm = TRUE)
  ...
  heatmap_genes_chrm_filt$Feature <- factor(
    heatmap_genes_chrm_filt$Feature,
    levels = chrm_filt_gene_order
  )
